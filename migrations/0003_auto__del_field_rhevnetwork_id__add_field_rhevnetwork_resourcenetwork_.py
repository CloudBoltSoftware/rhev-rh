...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # MySQL will complain if we delete the old column first because then
        # the table has zero columns:
        #
        #    django.db.utils.DatabaseError: (1090, "You can't delete all columns with ALTER TABLE; use DROP TABLE instead")
        #
        # We can't add the new column first because then:
        #
        #    django.db.utils.DatabaseError: (1090, "You can't delete all columns with ALTER TABLE; use DROP TABLE instead")
        #
        # So, we wrap the deletion of the old column and the creation of the
        # new column with the creation and deletion of a bogus column.

        db.add_column('rhev_rhevnetwork', 'bogus_field_name',
                models.IntegerField(null=True))

        # Deleting field 'RhevNetwork.id'
        db.delete_column('rhev_rhevnetwork', 'id')

        # Adding field 'RhevNetwork.resourcenetwork_ptr'
        db.add_column('rhev_rhevnetwork', 'resourcenetwork_ptr',
                      self.gf('django.db.models.fields.related.OneToOneField')(default=1, to=orm['resourcehandlers.ResourceNetwork'], unique=True, primary_key=True),
                      keep_default=False)

        db.delete_column('rhev_rhevnetwork', 'bogus_field_name')


    def backwards(self, orm):
        # Adding field 'RhevNetwork.id'
        db.add_column('rhev_rhevnetwork', 'id',
                      self.gf('django.db.models.fields.AutoField')(default=1, primary_key=True),
                      keep_default=False)

        # Deleting field 'RhevNetwork.resourcenetwork_ptr'
        db.delete_column('rhev_rhevnetwork', 'resourcenetwork_ptr_id')
